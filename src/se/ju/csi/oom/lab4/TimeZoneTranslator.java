...
public class TimeZoneTranslator {
	...
	static DateTime shiftTimeZone(DateTime inputDate, int fromOffset, int toOffset) {
		...
		if(targetHour < 0 || targetHour > 24) {
			int restHours = targetHour % 24;
			targetHour = restHours < 0 ? 24 + restHours : restHours;
			targetDay += restHours < 0 ? -1 : 1;
			if(targetDay < 1 || targetDay > 31) {
				targetMonth += targetDay < 1 ? -1 : 1;
				targetDay = targetDay < 1 ? 31 : 1;	
				if(targetMonth < 1 || targetMonth > 12) {				
					targetYear += targetMonth < 1 ? -1 : 1;
					targetMonth = targetMonth < 1 ? 12 : 0;			
				}
			}
